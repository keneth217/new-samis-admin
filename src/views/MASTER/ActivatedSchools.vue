  ...
  methods: {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          if (fullSchoolData) {
            ...
            this.selectedSchool = {
              ...
              // SchoolController audit fields (prefer school list API, then activation row fallback)
              registeredByID: fullSchoolData.registeredByID ?? school.registeredByID ?? null,
              registeredByName: fullSchoolData.registeredByName || school.registeredByName || '',
              updatedByID: fullSchoolData.updatedByID ?? school.updatedByID ?? school.handledByID ?? null,
              updatedByName: fullSchoolData.updatedByName || school.updatedByName || school.handledByName || '',
    ...
    exportToPdf() {
      ...
      try {
        ...
        const cols = ['#', 'School Name', 'School Code', 'Module Name', 'Install Date', 'Expiry Date', 'Selling Price', 'Maint Fee'];
        const colWidths = [8, 50, 22, 28, 26, 26, 22, 20];
        ...
        doc.text('Activated Schools', margin, y);
        y += 10;
        doc.setFontSize(8);
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `activated-schools-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.pdf`;
        link.click();
